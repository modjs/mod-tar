/**
 * Thanks to Isaac Schlueter's work on fstream-npm on which this file is based.
...
function Packer (props) {
  ...
  if (props.packageInfo && props.packageInfo.mod) {
      this.browserInclude = props.packageInfo.mod.include;
  }
...
Packer.prototype.applyIgnores = function (entry, partial, entryObj) {
  ...
  // if the package.json file has a mod.include property, *only* include
  ...
  if (this.browserInclude) {
      for (var i = 0; i < this.browserInclude.length; i++) {
        ...
        // test for subpaths, eg mod.include = ['foo'], entry = 'foo/bar.js'
        if (isSubPath(this.browserInclude[i], entry)) {
            return true;
        }
      }
      return !!(partial);
...
Packer.prototype.addIgnoreFiles = function () {
  ...
  // if there's a .tignore, then we do *not* want to
  // read the .gitignore.
  if (-1 !== entries.indexOf(".tignore")) {
...
Packer.prototype.emitEntry = function (entry) {
  ...
  // rename it to .tmignore in the output.
  if (entry.basename === ".gitignore") {
    entry.basename = ".tignore"
  ...
  if (entry.type !== "Directory") {
    ...
    , t = entry.path.substr(h.length + 1)
    , p = h + "/" + t
